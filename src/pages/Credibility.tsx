...
const testimonials = [
  { name: "Global Buyer - Dubai", text: "TREERT delivers consistent quality and perfect documentation every shipment." },
  { name: "Importer - Singapore", text: "Packaging, freshness, and compliance are always top class." },
  { name: "Wholesale Partner - Malaysia", text: "Very professional export process and reliable timelines." },
  { name: "Distributor - Qatar", text: "Quality control and certifications are always perfect." },
  { name: "Retail Chain - Oman", text: "One of the most trustworthy exporters we’ve worked with." },
];
    ...
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-green-50 via-white to-emerald-50">
      ...
      <section className="relative pt-24 pb-24 text-center">
        <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full glass mb-6">
          ...
          <span className="font-semibold text-green-800">Globally Trusted Exporter</span>
        </div>

        <h1 className="text-5xl md:text-6xl font-bold gradient-text mb-6">Our Credibility</h1>
      ...
      <section className="mb-28">
        <h2 className="text-3xl font-bold text-center mb-10">Recognized & Certified By</h2>
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="flex gap-10 w-max will-change-transform" ref={certRef}>
            {[...certificates, ...certificates].map((cert, i) => (
              <a key={i} href={cert.link} target="_blank" rel="noreferrer" className="w-64 shrink-0">
                <div className="gallery-card bg-white rounded-2xl p-6 shadow-lg hover:scale-105 transition-all duration-500">
                  <img src={cert.image} alt={cert.title} className="h-36 w-full object-contain" />
                  <p className="text-center mt-4 font-semibold text-gray-800">{cert.title}</p>
                </div>
              </a>
            ))}
          </div>
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="flex gap-8 w-max will-change-transform" ref={testRef}>
            {[...testimonials, ...testimonials].map((t, i) => (
              <div key={i} className="w-80 shrink-0 glass rounded-2xl p-8 shadow-xl border border-white/20">
